...
test('peer key trusts only one valid forwarded IP when explicitly enabled', () => {
  const forwardedRequest = createPeerRequest(' 198.51.100.42 ')
  assert.equal(getRequestPeerKey(forwardedRequest), '127.0.0.1')
  assert.equal(getRequestPeerKey(forwardedRequest, true), '198.51.100.42')
  assert.equal(
    getRequestPeerKey(createPeerRequest('2001:db8::42'), true),
    '2001:db8::42',
  )

  assert.equal(
    getRequestPeerKey(createPeerRequest('198.51.100.42, 203.0.113.7'), true),
    '127.0.0.1',
  )
  assert.equal(
    getRequestPeerKey(createPeerRequest(['198.51.100.42']), true),
    '127.0.0.1',
  )
  assert.equal(
    getRequestPeerKey(createPeerRequest('198.51.100.42:1234'), true),
    '127.0.0.1',
  )
})
