...
test('OAuth polling requires its distinct secret and consumes a completed session once', { timeout: 5000 }, async (context) => {
  ...
  await withOAuthServer(async ({ origin, issuedUsers }) => {
    ...
    assert.equal(callback.headers.location, '/oauth-result?result=success')
...
test('a second OAuth callback cannot race the callback already processing', { timeout: 5000 }, async (context) => {
  ...
  await withOAuthServer(async ({ origin }) => {
    ...
    assert.equal(duplicateCallback.statusCode, 303)
    assert.equal(duplicateCallback.headers.location, '/oauth-result?result=invalid')
    assert.equal(tokenExchangeCount, 1)

    releaseTokenExchange?.()
    const successfulCallback = await firstCallback
    assert.equal(successfulCallback.statusCode, 303)
    assert.equal(successfulCallback.headers.location, '/oauth-result?result=success')
...
test('a failed OAuth exchange deletes the session instead of leaving it pending', { timeout: 5000 }, async (context) => {
  ...
  await withOAuthServer(async ({ origin, issuedUsers }) => {
    ...
    assert.equal(callback.statusCode, 303)
    assert.equal(callback.headers.location, '/oauth-result?result=error')
...
test('OAuth start rate limiting separates valid forwarded peers only when proxy trust is enabled', { timeout: 5000 }, async (context) => {
  silenceExpectedLogs(context)
  ...
  await withOAuthServer(async ({ origin }) => {
    ...
    const limited = await requestLocal(origin, '/auth/discord/start', {
      'x-memedrop-key': 'server-test-key',
      'x-forwarded-for': '198.51.100.42',
    })
    assert.equal(limited.statusCode, 429)

    const otherPeer = await requestLocal(origin, '/auth/discord/start', {
      'x-memedrop-key': 'server-test-key',
