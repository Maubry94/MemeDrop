...
type DiscordOAuthResult = 'success' | 'invalid' | 'expired' | 'error'

const sendOAuthResultRedirect = (
  response: http.ServerResponse,
  result: DiscordOAuthResult,
) => {
  response.writeHead(303, {
    'cache-control': 'no-store',
    location: `/oauth-result?result=${result}`,
  })
  response.end()
}
...
export const createDiscordOAuthHandlers = ({
  ...
  const handleDiscordAuthStart = (
    ...
    if (!authStartLimiter.consume(getRequestPeerKey(request, trustProxy))) {
  ...
  const handleDiscordAuthCallback = async (
    ...
    if (!code || !sessionId || !session || session.status !== 'pending') {
      ...
      sendOAuthResultRedirect(response, 'invalid')
      return
    }
    ...
    try {
      ...
      if (
        ...
        sendOAuthResultRedirect(response, 'expired')
        return
      }
      ...
      sendOAuthResultRedirect(response, 'success')
    } catch (error) {
      if (authSessions.get(sessionId) === processingSession) {
        authSessions.delete(sessionId)
      }
      console.error('Connexion Discord OAuth impossible:', error)
      sendOAuthResultRedirect(response, 'error')
    }
  }
